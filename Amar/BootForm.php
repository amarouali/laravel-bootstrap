<?php 
	...
	class BootForm{
		...
		public function open($model, array $options=array()){
			...
			if ($model->id) {
				...
				if(!isset($options['action'])){
					$options['action']=["$model_name@update",$model->id];
				}
				
			}else{
				if(!isset($options['action'])){
					$options['action']=["$model_name@store"];
				}
		...
		public function checkbox($name, $value,$label=null,$check=null){
		...
		public function submit($name = null,$class = null){
			...
			return  '<button type="submit" class="btn btn-'.$class.'">'.$name.'</button>';
		}
