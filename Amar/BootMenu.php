...
**/
	class BootMenu{
		...
		function navBar(array $title ,$lists =array()){
			...
			$return .= 			'<a class="navbar-brand" href="'.$title['link'].'">'.($title["title"] ? $title["title"] : "").'</a>
						    </div>';
			...
			foreach ($list as  $v) {
							...
							if($show){
								if(isset($v['dropdown'])){
									...
									$li .='<li><a href="'.$v['link'].'">'.$v['title'].'</a></li>';									
								}
		...
		function dropDown($list){
			...
			$return .= '<a href="#" class="dropdown-toggle" data-toggle="dropdown">'.$list['title'].'<span class="caret"></span></a>';
